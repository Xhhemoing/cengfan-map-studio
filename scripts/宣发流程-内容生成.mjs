 ...
 *   zhihu      知乎回答骨架
 *
 * 人工定稿优先使用 docs/宣发/投放文案-*.md，本脚本只出草稿。
 *
 * 主题（theme）：
 *   painpoint  痛点唤醒
 *   result     成果展示
...
const TEMPLATES = {
    ...
    painpoint: [
      ...
      },
      {
        ...
        body: [
          ...
          '• 高清导出：PNG / PDF / 项目包',
          '',
          '【行动】评论扣 1 发示例表。开发者请走 Issue 模板提意见。',
          '',
  ...
  juejin: {
    painpoint: [
      {
        title: '用 React + d3-geo 做一张能导入 Excel 的毕业去向地图',
        body: [
          '开场：一张脱敏成品图 + 班主任场景（不要写成软文标题）。',
          '为什么不用现成图表库：要海报导出、省界锚点、手拖微调。',
          '实现 1：表头别名识别（指出 src/lib 导入模块）。',
          '实现 2：投影与卡片避让。',
          '实现 3：.cengfan 项目包。',
          '跑通：npm install && npm run dev',
          'CTA：到征求意见 Issue 回复导入/布局/导出哪步劝退，不要只求 Star。',
          '完整可复制稿：docs/宣发/投放文案-开发者社区.md'
        ]
      }
    ]
  },
  v2ex: {
    painpoint: [
      {
        title: '做了个毕业班去向地图编辑器（React + d3-geo），求导入和布局意见',
        body: [
          '节点建议：create 或 share。标题禁止「神器」。',
          '正文直接复制 docs/宣发/投放文案-开发者社区.md 第一节。',
          '发完 24 小时盯评论。'
        ]
      }
    ]
  ...
  zhihu: {
    painpoint: [
      {
        title: '回答「毕业墙 / 班级去向图怎么做」',
        body: [
          '不要新建广告问题，去搜已有问题。',
          '先写 Excel/PPT/手绘的坑，再给三列表头步骤，最后才提蹭饭图。',
          '完整骨架：docs/宣发/投放文案-用户侧.md 知乎一节。',
          '禁止升学率口径。'
        ]
      }
    ]
