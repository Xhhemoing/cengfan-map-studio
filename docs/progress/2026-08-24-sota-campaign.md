...
1. **AI**：瞬时失败只回滚 transcript，影子工程与步骤留下孤儿写入，重试会双重应用。助手允许并发跑第二会话，第一会话永久卡在 running；续聊会把用户取消勾选的步骤重新勾上。同文案两轮会把任务段钉在首次出现。
2. **协作**：Hook 未接 `onKicked`，被踢端 UI 仍显示已连接。心跳里的 `roomStore.get` 给房间自续命，TTL/`maxRooms` 失效。邀请无上限无限流。
3. **画布/导出**：嘉宾面板零位移点击提交空事务；SVG `downloadText` 同步 revoke；同 z 重叠不报 occlusion。
4. **导入**：工作台工程包入口绕过 24MB；字体去重丢 id 映射导致静默回落默认字体。名单 visibility 列标破坏性暂缓。
5. **工作台**：默认路径「导出工程」对话框只挂在旧编辑器分支，新模板路径静默空操作。设置全屏进出丢焦点。`createLocalStorageMirror` 死代码。

## 第 23 轮（文件所有权互斥）

| 切片 | 允许改动的路径 |
|---|---|
| 续聊回滚影子 | `src/lib/agent-session.ts` 及测试。失败时连同 shadow/_steps 回到入口快照。 |
| 禁并发会话 | `src/components/AgentAssistant.tsx` 及测试。任一会话 running 则拒绝第二 run；续聊只并入本轮新增步骤。 |
| 被踢 UI | `src/lib/useCollaborationRoom.ts` 及测试。订阅补 `onKicked`，复用已有移出收尾。 |
| 心跳不续命 + 邀请上限 | `server/collaboration.ts`、`server/index.ts`（心跳与 invitations 路由）及测试。心跳改 sweep/peek；邀请过期淘汰+每房上限。不要改 ping 事件本身。 |
