    ...
    <>
      ...
      {activePanel === "layout" && (
        <div className="panel-content">
          ...
          <button className="wide-button" type="button" onClick={onSaveTemplate}><Save size={16} aria-hidden /> 保存当前整体模板</button>
      ...
      {activePanel === "deliver" && (
        <div className="panel-content workflow-panel workflow-panel--deliver">
          ...
          <ActionGroup label="交付操作" className="workflow-delivery-actions">
            <button className="wide-button workflow-export-button" type="button" onClick={() => void posterExport.exportPng()} disabled={posterExport.exportingPng}><ImageDown size={16} aria-hidden />{posterExport.exportingPng ? "导出中..." : "导出 PNG"}</button>
