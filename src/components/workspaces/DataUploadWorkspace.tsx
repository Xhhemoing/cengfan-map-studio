...
import { provinceOptions } from "../../lib/roster-search-options";
...
/**
 * Center content of the DATA stage: the roster workbench header (title + live
 * roster status) and the roster/import table. The 数据质量 rail is a sibling
 * component (`DataUploadRail`) rendered by the unified right rail. Asset
 * management deliberately lives elsewhere: the content stage owns the asset
 * library and the map stage owns province textures.
 */
export function DataUploadWorkspace({
  ...
  return (
    <main className="data-upload-workspace data-upload-workspace--expanded" aria-label="名单工作台">
      <header className="data-upload-workspace__header">
        <div className="data-upload-workspace__title">
          <strong>名单</strong>
          <span>导入、录入、校验与地图映射</span>
        </div>
        <div className="data-upload-workspace__status" aria-label="名单状态">
...
/**
 * Right rail of the DATA stage: data quality only — the quality panel with the
 * map-mapping issue rows and province chips. The shell owns the rail chrome
 * (labelled aside + resizer + mobile drawer). The asset library is not part of
 * this stage; its primary entry is the content stage (素材库).
 */
export function DataUploadRail({
  ...
  return (
    <aside className="data-upload-workspace__rail" aria-label="数据工作台侧栏">
      <section className="data-upload-workspace__quality" aria-label="数据质量">
