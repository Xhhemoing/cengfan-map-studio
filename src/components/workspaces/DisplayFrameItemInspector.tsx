import type { DisplayFrameFixedItem, DisplayFrameFontWeight, DisplayFrameStyle } from "../../lib/display-frame";
import { resolveDisplayFrameItemPaint, resolveDisplayFrameSurface } from "../../lib/display-frame-style";
import { DeferredInput } from "../DeferredInput";

function weightToken(fontWeight: number): DisplayFrameFontWeight {
  if (fontWeight >= 700) return "bold";
  return fontWeight >= 500 ? "medium" : "normal";
}
...
export function DisplayFrameItemInspector({
  ...
  const paint = resolveDisplayFrameItemPaint(item, resolveDisplayFrameSurface(frameStyle));
  const updateStyle = (patch: NonNullable<DisplayFrameFixedItem["style"]>) => onChange({ style: { ...style, ...patch } });
  return (
    <section className="display-frame-item-inspector" aria-label="当前图层属性" data-display-frame-item-kind={item.kind}>
      ...
      {item.kind !== "decoration" && (
        <div className="display-frame-item-inspector__grid">
          <label htmlFor="display-frame-item-font-size">字号<DeferredInput id="display-frame-item-font-size" type="number" min={8} max={240} value={paint.fontSize} onCommit={(value) => updateStyle({ fontSize: Number(value) })} /></label>
          <label htmlFor="display-frame-item-color">文字色<DeferredInput id="display-frame-item-color" type="color" value={paint.color} onCommit={(value) => updateStyle({ color: value })} /></label>
          <label htmlFor="display-frame-item-weight">字重<select id="display-frame-item-weight" value={weightToken(paint.fontWeight)} onChange={(event) => updateStyle({ fontWeight: event.target.value as DisplayFrameFontWeight })}><option value="normal">常规</option><option value="medium">中等</option><option value="bold">粗体</option></select></label>
          <label htmlFor="display-frame-item-align">对齐<select id="display-frame-item-align" value={paint.align} onChange={(event) => updateStyle({ align: event.target.value as "left" | "center" | "right" })}><option value="left">左对齐</option><option value="center">居中</option><option value="right">右对齐</option></select></label>
        </div>
      )}
      {item.kind === "decoration" && (
        <div className="display-frame-item-inspector__grid">
          <label htmlFor="display-frame-item-color">描边色<DeferredInput id="display-frame-item-color" type="color" value={paint.color} onCommit={(value) => updateStyle({ color: value })} /></label>
          <label htmlFor="display-frame-item-stroke">描边宽<DeferredInput id="display-frame-item-stroke" type="number" min={0} max={24} value={paint.strokeWidth} onCommit={(value) => updateStyle({ strokeWidth: Number(value) })} /></label>
