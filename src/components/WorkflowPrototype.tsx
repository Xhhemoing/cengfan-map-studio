...
function PanelTitle({ label, detail }: { label: string; detail: string }) {
  return <header className="prototype-panel-title"><div><p>{detail}</p><h2>{label}</h2></div><button type="button" aria-label={`了解${label}`}><CircleHelp size={16} aria-hidden /></button></header>;
}
  ...
  return <nav className="prototype-steps" aria-label="海报制作流程">
    {areas.map(({ id, label, hint }, index) => (
      <button key={id} type="button" aria-label={label} aria-current={active === id ? "step" : undefined} onClick={() => onChange(id)}>
        <span className="prototype-step-number">{active === id ? <Check size={14} aria-hidden /> : index + 1}</span>
...
function LeftPanel({ active }: { active: AreaId }) {
  if (active === "roster") return <section className="prototype-side-panel"><PanelTitle label="名单检查" detail="准备数据" /><div className="prototype-import-actions"><button type="button"><FileSpreadsheet size={16} aria-hidden />上传表格</button><button type="button"><Plus size={16} aria-hidden />手动添加</button></div><div className="prototype-status-note"><Check size={15} aria-hidden /><span>16 个城市已定位，2 条海外去向单独保留。</span></div><div className="prototype-roster-list">{students.map(([name, university, city]) => <div key={name}><span className="prototype-avatar">{name.slice(0, 1)}</span><p><strong>{name}</strong><small>{university} · {city}</small></p><Check size={15} aria-hidden /></div>)}</div><button className="prototype-text-action" type="button">查看全部 18 条名单</button></section>;
  if (active === "map") return <section className="prototype-side-panel"><PanelTitle label="地图表达" detail="选择读图方式" /><div className="prototype-mode-list">{mapModes.map((mode, index) => <button key={mode} type="button" className={index === 0 ? "is-selected" : ""}><span>{mode}</span><small>{index === 0 ? "按省份汇总名单" : index === 4 ? "突出人数分布" : "实时切换"}</small></button>)}</div><div className="prototype-action-block"><p>浙江省已选中</p><button type="button"><Image size={16} aria-hidden />应用省份贴图</button></div></section>;
  if (active === "layout") return <section className="prototype-side-panel"><PanelTitle label="版式结构" detail="让信息均衡可读" /><div className="prototype-template"><div className="prototype-template-preview"><i /><i /><i /></div><div><strong>风景插画版</strong><small>1500 × 1000 · 横向</small></div><ChevronDown size={16} aria-hidden /></div><div className="prototype-layout-options"><label><span>卡片样式</span><b>紧凑 <ChevronDown size={14} aria-hidden /></b></label><label><span>连接线</span><b>折线 <ChevronDown size={14} aria-hidden /></b></label><label><span>画布背景</span><b><i className="prototype-color-dot" />暖白</b></label></div><button className="prototype-primary-action" type="button"><WandSparkles size={16} aria-hidden />重新智能排版</button><p className="prototype-footnote">系统会避开标题和地图内容区；移动单张卡片不会影响其他卡片。</p></section>;
  if (active === "content") return <section className="prototype-side-panel"><PanelTitle label="海报内容" detail="补充班级记忆" /><div className="prototype-content-actions"><button type="button"><Type size={16} aria-hidden />新增文字</button><button type="button"><UsersRound size={16} aria-hidden />添加嘉宾</button></div><div className="prototype-content-list"><button type="button">主标题 <small>我们的毕业去向</small></button><button type="button">副标题 <small>山高水长，来日再聚</small></button><button type="button">特别备注 <small>2026 届高三（2）班</small></button></div></section>;
  if (active === "assets") return <section className="prototype-side-panel"><PanelTitle label="省份贴图" detail="浙江省 · 已筛选" /><button className="prototype-upload" type="button"><Upload size={16} aria-hidden />上传浙江素材</button><div className="prototype-asset-grid">{["西湖", "龙井", "烟雨", "水墨"].map((asset, index) => <button key={asset} type="button" className={index === 0 ? "is-selected" : ""}><span className={`prototype-asset-art art-${index}`} /><strong>{asset}</strong></button>)}</div><button className="prototype-text-action" type="button">管理全部素材与字体</button></section>;
  return <section className="prototype-side-panel"><PanelTitle label="交付检查" detail="准备输出" /><div className="prototype-delivery-status"><div><Check size={16} aria-hidden /><span><strong>名单完整</strong><small>18 人已显示</small></span></div><div><Check size={16} aria-hidden /><span><strong>版式已检查</strong><small>未发现卡片遮挡</small></span></div><div><span className="prototype-warning-dot" aria-hidden="true">!</span><span><strong>2 条海外去向</strong><small>不会显示在中国地图</small></span></div></div><button className="prototype-primary-action" type="button"><ArrowDownToLine size={16} aria-hidden />导出 2× PNG</button><button className="prototype-secondary-action" type="button">保存到本机</button></section>;
}
...
export function WorkflowPrototype() {
  ...
  return <div className="workflow-prototype"><header className="prototype-header"><div className="prototype-brand"><span className="prototype-brand-mark"><MapPinned size={18} aria-hidden /></span><span><strong>蹭饭地图工作室</strong><small>流程工作台 · 原型</small></span></div><WorkflowTabs active={active} onChange={setActive} /><div className="prototype-header-actions"><button type="button" className="prototype-ai"><Sparkles size={16} aria-hidden />AI 助手</button><button type="button" className="prototype-export">导出</button></div></header><div className="prototype-workspace"><LeftPanel active={active} /><Poster selection={selection} onSelect={setSelection} /><Inspector selection={selection} /></div></div>;
}
