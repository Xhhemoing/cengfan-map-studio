 ...
 *
 * This lives in `src/lib` rather than next to the canvas because both callers must agree:
 * `PosterCanvas` uses it for the interactive layout, and the AI `auto_layout` tool uses it
 * through `agent-session-layout.ts`. When the agent kept its own obstacle set, the two
 * disagreed — the agent saw no texts and no decorations at all — and re-running auto-layout
 * from the assistant buried elements the canvas had carefully avoided.
 */
