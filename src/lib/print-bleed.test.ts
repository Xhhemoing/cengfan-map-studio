...
import { createDefaultScene, normalizeScene, updateSceneTarget } from "./scene-document";
...
describe("scene canvas print bleed field", () => {
  it("defaults to 0 and clamps patched values through normalizeScene", () => {
    const scene = createDefaultScene("original");

    expect(scene.canvas.printBleedMm).toBe(0);
    expect(normalizeScene(scene).canvas.printBleedMm).toBe(0);
    expect(normalizeScene({ ...scene, canvas: { ...scene.canvas, printBleedMm: undefined } }).canvas.printBleedMm).toBe(0);
    expect(normalizeScene({ ...scene, canvas: { ...scene.canvas, printBleedMm: -2 } }).canvas.printBleedMm).toBe(0);
    expect(normalizeScene({ ...scene, canvas: { ...scene.canvas, printBleedMm: 3 } }).canvas.printBleedMm).toBe(3);
    expect(normalizeScene({ ...scene, canvas: { ...scene.canvas, printBleedMm: 500 } }).canvas.printBleedMm)
      .toBe(MAX_PRINT_BLEED_MM);
    expect(updateSceneTarget(scene, { type: "canvas" }, { printBleedMm: 5 }).canvas.printBleedMm).toBe(5);
  });
});
