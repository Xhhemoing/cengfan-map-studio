...
describe("fullwidth colon separated rows", () => {
  ...
  it("splits a row typed with the vertical colon Word writes down a CJK column", () => {
    // ︰ (U+FE30) is the colon a vertically written Word table ships — neither ：
    // nor its small form ﹕ — so a row typed with it stayed a single field.
    const rawLine = "林舟︰北京大学︰北京市";
    const result = parseStudentText(rawLine);
  ...
  it("reads a roster headed by the vertical colon and names the gap of its empty 院校 cell", () => {
    const result = parseStudentText([
      "姓名︰院校︰城市︰去向类型",
      "苏禾︰浙江大学︰杭州市︰",
      "周晴︰哈佛大学︰波士顿︰海外",
  ...
  it("splits a row typed with the vertical form of the fullwidth colon", () => {
    // ︓ (U+FE13) is the other colon a vertical CJK export ships.
    const rawLine = "苏禾︓浙江大学︓杭州市";

    expect(parseStudentText(rawLine).candidates).toEqual([
      { name: "苏禾", university: "浙江大学", city: "杭州市", sourceLine: 1, rawLine },
    ]);
  });
});
