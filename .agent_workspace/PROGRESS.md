...
**Merge-all overlay (2026-08-25):** isolation `cursor/merge-all-branches-e17a` (fast models only). See `round-merge-all-r1.md`.  
...
| 11 | CLOSED | R11-1–R11-9 + fable/R11-10 | — | 264/1969/2; lint 0/1; fable 7 ACCEPT + 2 NITS; no blocker. |
| 12 | IN_PROGRESS | — | — | App ≤940; eight test splits; lint-green-per-merge. |
...
- Fable: `.agent_workspace/round-8-review.md`. 9 ACCEPT + 1 ACCEPT-WITH-NITS, no blocker. Briefing: `.agent_workspace/round-8-briefing.md`.
- CI: push + pull_request green on `553d5c1`.

## Round 9 notes

- Queue on `server/index.ts`: R9-1 → R9-2 → R9-7. Chain: R9-2 → R9-3. Proof R9-4 after R9-1+R9-2. App.test: R9-5 → R9-9. Independent: R9-6, R9-8. R9-10 last.
- Headline: transactions ack has no persistence fields; two writers orphan `.tmp` on rename failure.
...
- CI: push + pull_request green on `7834c47`.
...
- R10-1 (`964a641` / `6eba6e5`+`d730f69`): AI state store unlinks `.tmp` on rename failure; `.corrupt-*` capped at newest 5. Inline catch/`rm` (no import from room-snapshot-store). Rollback: revert `964a641`.
- R10-6 (`7d2717f` / `3c4c771`): `useCollaborationRoom.test.tsx` 1265→201 + harness + 4 domain files, all ≤400. Allowlist entry deleted. Rollback: revert `7d2717f`.
- R10-7 (`4010acc` / `931c310`): `DataWorkspace.test.tsx` 1362→241 + harness + 3 domain files, all ≤400. Product frozen. Allowlist entry deleted. Rollback: revert `4010acc`.
- R10-2 (`bd65d04` / `43cbd91`+`f896e5c`): `sweepStaleTemporaryFiles` once in `createReadyAiServer` before any load/writer. Boot-order pin lives in `server/boot-tmp-sweep.test.ts` so `index.test.ts` stays 2504; `index.ts` stays 1547. Rollback: revert `bd65d04`. Must land before R10-3.
- Allowlist after those four: 43 entries (was 45).
- R10-3 (`b5e17df` / `74ed0c0`+`e538d63`): static-file cluster → `server/static-files.ts`; `sendJson`/`corsHeaders` moved with it to avoid a cycle. index.ts 1547→1367; index.test.ts 2504→2378. Rollback: revert `b5e17df`. Must land before R10-4.
...
- Fable (`f6f5285`): 7 ACCEPT + 2 ACCEPT-WITH-NITS, **no merge blocker**. Review: `round-11-review.md`. Briefing: `round-11-briefing.md`.
